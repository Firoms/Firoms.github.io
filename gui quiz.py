...
class GameScreen:
    ...
    def placingComponenents(self):
        title = Label(self.display, text = "롤 챔프 맞추기 게임", background = "pink", font=("맑은 고딕", 20), height= 3)
        title.pack() #위에서 부터 보여줌

        describe = Label(self.display, text = "사진을 보고 캐릭터의 이름을 맞추는 게임입니다.\n 얼마나 많이 맞출 수 있는지 확인해보세요"
                         , font = ("맑은고딕", 12))
        describe.place(x=30, y=180)

        game_start = Button(self.display, fg = "green", text = "Game Start", height = 2, width =12, background = "yellow", command = self.gamestart)
        game_start.pack()
        ...
        return {'title_label':title, 'describe_label':describe, 'game_start_button' : game_start, 'img_label' :img_label,
                'play_entry':playField, 'life_label' : life_label}
