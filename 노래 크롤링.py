...
base_URL ='https://music.naver.com/listen/top100.nhn?domain=TOTAL_V2&page=' # 노래 차트 페이지 주소
...
for i in range(1,3) : #1~2 페이지에서
    ...
    song_list = soup.find_all("td",{"class": 'name'})[1:]  # 노래를 모두 가져옴 (단, 2번째부터 가져옴, 1번째는 이상한 코드가 나와 임의로 수정함)
    ...
    for single_id in track_id : # 노래번호들중 번호 하나씩
        num = (single_id['class'][-1]) #클래스가 여러개인 경우 클래스중 몇번째를 가져올지 선택해야하는데 맨 뒤에 번호가 있기 때문에
...
for id in id_list  : #기준으로 노래 변경 / id list에 저장되어 있는 id 숫자를 이용하여 노래를 변경한다.
    ...
    for single_row in lyric : # 가사 코드 한줄마다
        try:
            single_row = single_row.replace("<br/>","\n") # 코드에 <br/>은 엔터역할이기에 이것을 \n으로 replace 해주었다.
            # print(single_row)
    ...
    with open(f'{title}.txt','w',-1,"utf-8") as f : # 파일로 저장하는 과정인데, 한국어는 오류가나서 -1,"utf-8"을 붙여 해결하였다ㅏ.
